...
function GroupLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-500 px-1">{children}</p>
  )
}
    ...
    <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
      <div>
        <h2 className="font-semibold text-gray-800 dark:text-gray-100 flex items-center gap-2">
          <Wifi size={16} className="text-brand-600" /> Live Updates
        </h2>
...
export default function General() {
  ...
  const [labelPrintQty, setLabelPrintQty] = useState<1 | 2>(1)
  ...
  useEffect(() => {
    ...
    if (settings?.label_print_quantity === '2') setLabelPrintQty(2)
    else if (settings?.label_print_quantity === '1') setLabelPrintQty(1)
  }, [settings])
  ...
  const savePrinterMutation = useMutation({
    mutationFn: () => Promise.all([
      setSetting('label_printer_name', labelPrinter),
      setSetting('label_print_quantity', String(labelPrintQty)),
    ]),
  ...
  async function handleTestPrint() {
    ...
    setTestError('')
    try {
      const res = await fetch(`/api/print/test?printer=${encodeURIComponent(labelPrinter)}&qty=${labelPrintQty}`, { method: 'POST' })
      if (!res.ok) {
  ...
  return (
    <div className="p-6 max-w-xl space-y-10">
      ...
      <div className="space-y-3">
        ...
        <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">Theme</h2>
      ...
      <div className="space-y-3">
        ...
        <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">Spoolman</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              Connect to your Spoolman instance to pull live filament stock into the forecast.
            </p>
          </div>
          <div>
            <label className="text-xs text-gray-500 dark:text-gray-400 block mb-1">URL</label>
            <input
              type="url"
        ...
        <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">Square</h2>
      ...
      {/* ── Cost Accounting ── */}
      <div className="space-y-3">
        <GroupLabel>Cost Accounting</GroupLabel>

        <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">Machine Cost</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              Default cost per hour for machine time. Can be overridden per printer type.
            </p>
          </div>
        ...
        <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
            <h2 className="font-semibold text-gray-800 dark:text-gray-100">Electricity Cost</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
        ...
        <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
              Applied to total cost to calculate the Suggested MSRP. A value of 1.2 means 20% above cost.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <input
      ...
      {/* ── Printers ── */}
      <div className="space-y-3">
        <GroupLabel>Printers</GroupLabel>
        ...
        <section className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-4">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">Default quantity</label>
            <div className="flex gap-3">
              {([1, 2] as const).map(n => (
                <button
                  key={n}
                  onClick={() => setLabelPrintQty(n)}
                  className={clsx(
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500">
              {labelPrintQty === 2 ? 'Prints two labels per spool — one for each side.' : 'Prints one label per spool.'}
            </p>
          </div>
